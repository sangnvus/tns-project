...
public class CarMakerDAOImpl extends BaseDAOImpl implements CarMakerDAO{
	...
	public CarMaker findById(java.lang.String id) {
		...
		try {
			CarMaker instance = (CarMaker) getSessionFactory().getCurrentSession()
					.get("vn.co.taxinet.orm.CarMaker", id);
			if (instance == null) {
	...
	public List<CarMaker> findByExample(CarMaker instance) {
		...
		try {
			List<CarMaker> results = (List<CarMaker>) getSessionFactory()
					.getCurrentSession()
					.createCriteria("vn.co.taxinet.orm.CarMaker")
	...
	/* (non-Javadoc)
	 * @see vn.co.taxinet.dao.CarMakerDAO#selectAllCarMaker()
	 */
	public List<CarMaker> selectAllCarMaker() {
		Session session = getSessionFactory().getCurrentSession();
		String hql = "FROM CarMaker";
		Query query = session.createQuery(hql);
		List<CarMaker> carMakerList = query.list();
		return carMakerList;
	}
}
